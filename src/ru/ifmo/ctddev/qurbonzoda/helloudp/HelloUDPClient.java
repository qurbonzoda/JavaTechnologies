package ru.ifmo.ctddev.qurbonzoda.helloudp;
...
public class HelloUDPClient implements HelloClient {
    ...
    private static void printUsage() {
        ...
        System.out.println("Usage:\n\tHelloUDPClient <server name or ip-address> <server port number> <query prefix>"
                + " <number of parallel threads> <number of queries in each thread>");
    }
    ...
    private static class ConnectResolver implements Runnable {
        ...
        public void run() {
            try (DatagramSocket serverSocket = new DatagramSocket()) {
                    ...
                    while (true) {
                        try {
                            ...
                            String queryString = new String(
                                    queryPacket.getData(), queryPacket.getOffset(), queryPacket.getLength());
                            String responseString = new String(
                                    responsePacket.getData(), responsePacket.getOffset(), responsePacket.getLength());

                            int expectedResponseLength = queryString.length() + RESPONSE_PREFIX.length();
                            if (responseString.length() == expectedResponseLength
                                    && RESPONSE_PREFIX.equals(responseString.substring(0, RESPONSE_PREFIX.length()))
                                    && queryString.equals(responseString.substring(RESPONSE_PREFIX.length()))) {
                ...
                System.err.println("Exception occurred while working on "
                        + Thread.currentThread().getName() + ", cause: " + e.getMessage());
            }
        }
    }
}