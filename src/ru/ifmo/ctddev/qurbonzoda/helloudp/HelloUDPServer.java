...
public class HelloUDPServer implements HelloServer {
    ...
    public void start(int portNumber, int threadNumber) {
        try {
            ...
            System.err.println("Error occurred while creating HelloUDPServer on the given port, cause: "
                    + e.getMessage());
        }
    }
        ...
        public void run() {
            while (!serverSocket.isClosed()) {
                ...
                try {
                    ...
                    String request = new String(receivePacket.getData(), receivePacket.getOffset(),
                            receivePacket.getLength());
                    String response = RESPONSE_PREFIX + request;

                    byte[] responseBytes = response.getBytes();
                    DatagramPacket responsePacket = new DatagramPacket(
                            responseBytes, responseBytes.length, receivePacket.getSocketAddress());

                    serverSocket.send(responsePacket);
                } catch (SocketException e) {
                    // socket closed. work finished
                } catch (IOException e) {
                    System.err.println("Exception while working on " + Thread.currentThread().getName()
                            + ", cause: " + e.getMessage());
                }
            }
        }
